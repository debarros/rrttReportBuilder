...
exportUpdate.REPORT = function(uploadFilePath, report) {
  ...
  scoreupdatefilepath = paste0(report$getDataLocation(),"\\","ScoreUpdates.txt")
  ...
  if(nrow(NewScores) > 0){ #If there are new scores, write a new score update
    ...
    write(out, scoreupdatefilepath)  
  } else {
    if(file.exists(scoreupdatefilepath)){file.remove(scoreupdatefilepath)} # /if the score update text file exists
  } # /if-else
} # /function
